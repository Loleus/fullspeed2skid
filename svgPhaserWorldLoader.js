...
export async function loadSVGPhaserWorld(svgUrl, worldSize = 4096, tileSize = 256) {
  ...
  const [bgImg, asphaltImg, cobblestoneImg] = await Promise.all([
    loadImage(`assets/${bgTexture}.jpg`).catch(() => null),
    loadImage('assets/asphalt.jpg').catch(() => null),
    loadImage('assets/cobblestone.jpg').catch(() => null)
  ]);
  ...
  if (obstaclesGroup) {
    ...
    for (const obs of obstacles) {
      ...
      if (!texImg) {
        try {
          texImg = await loadImage(`assets/${texName}.jpg`);
