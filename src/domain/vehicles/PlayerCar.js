...
//     // Przyspieszenie i opory – CZĘŚĆ 1: tylko napęd (bez oporu powietrza)
//     let force;
//     if (throttle >= 0) {
//       force = throttle * this.accel;
//     } else {
...
//     // Ograniczanie prędkości zależnie od kierunku (limit „twardy” – maksymalna dopuszczalna prędkość auta)
//     if (this.v_x >= 0) {
//       this.v_x = Phaser.Math.Clamp(this.v_x, 0, localMaxSpeed);
//     } else {
//       this.v_x = Phaser.Math.Clamp(this.v_x, -localMaxRevSpeed, 0);
...
//     // --- NOWY, „MIĘKKI” MODEL OPORU POWIETRZA I TOCZENIA ---

//     // Napęd przeliczony na siłę (jak wcześniej)
//     let engineForce = throttle >= 0 ? throttle * this.accel : throttle * this.revAccel;

//     // Aerodynamiczny opór powietrza ~ v^2 (zachowujemy Twój wzór)
//     let F_drag = this._dragConst * this.v_x * Math.abs(this.v_x);

//     // Opór toczenia
//     let F_roll = this.rollingResistance * this.carMass * this.gravity * Math.sign(this.v_x);

//     // Celem jest: opór ma spowalniać PRZYSPIESZENIE coraz mocniej wraz z prędkością,
//     // ale nie zmniejszać / nie blokować „twardego” localMaxSpeed.
//     //
//     // Zamiast traktować F_drag jako dodatkowy hamulec, ograniczamy jego wpływ
//     // na netto‑siłę, im bliżej maksymalnej prędkości jesteśmy.
//     //
//     // factor = 0 przy v=0  -> pełna moc oporu (duże „zduszenie” przyspieszenia od początku)
//     // factor -> 1 przy v->localMaxSpeed -> opór praktycznie nie odejmuje prędkości,
//     //                                      tylko „wygina” krzywą przyspieszenia.

//     let vxSign = Math.sign(this.v_x) || (throttle >= 0 ? 1 : -1);
//     let currentMax = vxSign >= 0 ? localMaxSpeed : localMaxRevSpeed;
//     let speedRatio = Phaser.Math.Clamp(Math.abs(this.v_x) / (currentMax || 1), 0, 1);

//     // Im bliżej maksymalnej prędkości, tym słabiej opór „hamuje”.
//     // Ten wykładnik (2) można stroić (1..4) – im wyższy, tym mocniej opór działa przy niskich prędkościach,
//     // a łagodniej przy wysokich (bardziej sprężynowo).
//     let dragInfluence = 1 - Math.pow(speedRatio, 2); // 1 przy 0 prędkości, 0 przy v==max

//     // Zastosuj tylko część F_drag do odejmowania od siły netto.
//     let F_drag_effective = F_drag * dragInfluence;

//     // Opór toczenia zostawiamy „pełny”, bo jest prawie stały (mały) i nie niszczy maksymalnej prędkości.
//     let F_net = engineForce - F_drag_effective - F_roll;

//     // Zastosuj wynikową siłę do prędkości
//     this.v_x += (F_net / this.carMass) * dt;

//     // Nie pozwól wyjść ponad „twarde” limity auta.
//     if (this.v_x >= 0) {
//       this.v_x = Phaser.Math.Clamp(this.v_x, 0, localMaxSpeed);
//     } else {
//       this.v_x = Phaser.Math.Clamp(this.v_x, -localMaxRevSpeed, 0);
...
export class PlayerCar extends Car {
  ...
  updatePhysics(dt, steerInput, throttle, surface) {
    ...
    // **ZMIANA TU: OPÓR POWIETRZNY Z OPÓWIEĐZIĄ PRZYŚPIESZENIA**
    let F_roll = this.rollingResistance * this.carMass * this.gravity * Math.sign(this.v_x);

    // Używamy odpowiedniej maksymalnej prędkości w zależności od kierunku ruchu
    const currentMaxSpeed = this.v_x >= 0 ? localMaxSpeed : localMaxRevSpeed;

    // Oblicz opór powietrzany tak, aby przyspieszenie stopniowo spadało, a auto osiągnęło max prędkość
    // Gwarantuje, że przy pełnym gazie auto osiągnie currentMaxSpeed, a przyspieszenie maleje stopniowo
    let speedFactor = Math.abs(this.v_x) / currentMaxSpeed; // Współczynnik w przedziale [0, 1]
    
    // Opór rośnie kwadratowo ze prędkością, ale jest skalowany tak, aby nie blokować max prędkości
    // Wartość współczynnika dobiera się, aby przyspieszenie spadało płynnie
    let F_drag = this._dragConst * speedFactor * Math.abs(this.v_x) * this.v_x;
    ...
    // Ogranicz prędkość wektorową do currentMaxSpeed (dla przodu) oraz do localMaxRevSpeed (dla tyłu)
    let forwardSign = Math.sign(this.v_x) || 1; // jeśli v_x == 0, traktuj jako przód
    let allowedMax = forwardSign >= 0 ? localMaxSpeed : localMaxRevSpeed;
    let speed = Math.hypot(this.v_x, this.v_y);
    if (speed > allowedMax) {
