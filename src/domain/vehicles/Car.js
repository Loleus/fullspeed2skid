...
//     this.currentFrameLerp = 0;
//     this.frameLerpSpeed = 6; // Szybkość interpolacji (większa wartość = szybsza animacja)
...
//   updateVisualSpriteFromAngle(dt) {

//     const vs = this.visualSprite;
//     const s = this.carSprite;
//     if (!vs || !s || !vs.texture) return;

//     // Ustaw pozycję wizualnego sprite'a na równe fizycznemu
//     vs.x = s.x;
//     vs.y = s.y;
...
//     // Ustalamy liczbę klatek kierunkowych (0-23, bez powtarzającej się klatki 24)
//     const dirFrames = totalFrames >= 24 ? 24 : totalFrames;
//     const stepDeg = 360 / dirFrames; // Kąt między kolejnymi klatkami (dla 24 klatek to 15°)

//     // Normalizacja kąta do systemu gdzie 0° = góra (pasuje do arkusza sprite'ów)
...
//     // Oblicz docelową klatkę
//     let targetFrameIndex = Math.round(angleDeg / stepDeg);
//     if (targetFrameIndex >= dirFrames) targetFrameIndex = 0; // Wykluczamy klatkę 24

//     // Logika płynnych przejść
//     if (this.targetFrameIndex !== targetFrameIndex) {
//       // Oblicz najkrótszą ścieżkę między klatkami (unikamy długich obrotów)
//       let frameDiff = targetFrameIndex - this.targetFrameIndex;
//       if (frameDiff > dirFrames / 2) frameDiff -= dirFrames;
//       else if (frameDiff < -dirFrames / 2) frameDiff += dirFrames;

//       // Przelicz kąt obrotu dla aktualnej klatki
//       const angleDiffRad = Phaser.Math.DegToRad(frameDiff * stepDeg);

//       // Interpoluj w kierunku docelowej klatki
//       this.currentFrameLerp += this.frameLerpSpeed * dt;
//       if (this.currentFrameLerp >= 1) {
//         // Osiągnięto docelową klatkę - resetuj stan
//         this.targetFrameIndex = targetFrameIndex;
//         this.currentFrameLerp = 0;
//         vs.rotation = 0;
//       } else {
//         // Obracaj aktualną klatkę aby zminimalizować skok
//         vs.rotation = angleDiffRad * this.currentFrameLerp;
//       }
//     } else {
//       // Bez zmiany klatki - resetuj interpolację i obrót
//       this.currentFrameLerp = 0;
//       vs.rotation = 0;
//     }

//     // Ustaw aktualną klatkę wizualnego sprite'a
//     vs.setFrame(this.targetFrameIndex);

//     /* 
//     // Opcjonalny log do debugowania
//     console.log(
//       'Docelowa klatka:', targetFrameIndex,
//       'Aktualna klatka:', this.targetFrameIndex,
//       'Interpolacja:', this.currentFrameLerp.toFixed(2),
//       'Obrót:', Phaser.Math.RadToDeg(vs.rotation).toFixed(1) + '°'
//     );
//     */
//   }
// }
...
export class Car {
  constructor(scene, carSprite, worldData) {
    ...
    this.frameTransitionSpeed = 6;    // Szybkość przejścia (większa = szybciej)
  ...
  updateVisualSpriteFromAngle(dt = 0) {
    ...
    const stepDeg   = 360 / dirFrames;   // np. 15°
    const halfStep  = stepDeg / 2;       // np. 7.5°
