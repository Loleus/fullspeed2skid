...
export class MenuUI {
  constructor(scene) {
    this.scene = scene;
    this.menuButtons = [];
    this.gradientState = { stop1: 0.2, stop2: 0.4 };
    this.gradientTween = null;
    this.gradientCanvas = null;
    this.logo = null;
  ...
  updateGradient(ctx, width, height) {
    const g = ctx.createLinearGradient(0, 0, 0, height);
    g.addColorStop(0, 'rgba(26, 26, 41, 1)');
    g.addColorStop(this.gradientState.stop1, 'rgba(43, 82, 114, 0)');
    g.addColorStop(this.gradientState.stop2, 'rgba(43, 82, 114, 0)');
    g.addColorStop(1, 'rgba(26, 26, 41, 1)');
    ctx.clearRect(0, 0, width, height);
    ctx.fillStyle = g;
    ctx.fillRect(0, 0, width, height);
