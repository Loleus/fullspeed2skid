  ...
  async create() {
    ...
    buttons.forEach(btn => {
      ...
      const text = this.add.text(0, 0, btn.label, {
        fontFamily: ff,
        fontSize: fs,
        color: btn.disabled ? dc : tc,
        align: 'center',
        padding: { left: p, right: p, top: p, bottom: p }
      }).setOrigin(0.5).setShadow(2, 2, '#000', 3, false, true);

      const container = this.add.container(x + w / 2, y, [shadow, bg, text]);
      ...
      if (!btn.disabled) {
        ...
        hitCircle.on('pointerover', () => this.drawButton(bg, hf, a, s, w, h));
        hitCircle.on('pointerout', () => this.drawButton(bg, f, a, s, w, h));
        hitCircle.on('pointerdown', () => this.drawShadow(shadow, sp, w, h, sf, a));
